...
# Modified version to be able to specify the network address as a parameter of network and not the network name itself #

DOCUMENTATION = '''
---
module: vmware_guest_mod
...
#
   - name: Clone template and customize
     vmware_guest:
       ...
       networks:
         '192.168.1.0/24':
           # Added parameter network_address
            ...
            if self.params['ips']:
                for ip_string in self.params['ips']:
                    ...
                    for network in self.params['networks']:
                        if network:
                            if self.params['networks'][network]['network_address']:
                                if ip in  IPNetwork(self.params['networks'][network]['network_address']):
                                    self.params['networks'][network]['ip'] = str(ip)
                                    ipnet = IPNetwork(self.params['networks'][network]['network_address'])
                                    self.params['networks'][network]['subnet_mask'] = str(
                                      ipnet.netmask
                                    )
                                    ip_settings.append(self.params['networks'][network])
                            elif ip in IPNetwork(network):
                                self.params['networks'][network]['ip'] = str(ip)
